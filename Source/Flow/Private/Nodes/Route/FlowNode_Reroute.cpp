...
bool UFlowNode_Reroute::EvaluateAndGetOutputValue(const FName OutputPinName, FProperty*& OutProperty, const void*& OutDataPtr)
{
	OutProperty = nullptr;
	OutDataPtr = nullptr;
	...
	{
		LogError(FString::Printf(TEXT("Reroute node %s has no input connection to forward data from."), *GetNameSafe(this)));
		return false;
	}
	...
	{
		LogError(FString::Printf(TEXT("Reroute node %s has invalid input connection data."), *GetNameSafe(this)));
		return false;
	}
	...
	if (!SourceNode)
	{
		LogError(FString::Printf(TEXT("Reroute node %s could not find source node %s."), *GetNameSafe(this), *SourcePinInfo.NodeGuid.ToString()));
		return false;
	}
...
void UFlowNode_Reroute::CachePinProperties()
{
	// Reroute nodes don't cache properties themselves
}
